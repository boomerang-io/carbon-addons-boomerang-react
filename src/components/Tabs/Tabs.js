...
export default class Tabs extends React.Component {
  ...
  getEnabledTabs = () =>
    React.Children.toArray(this.props.children).reduce(
      (enabledTabs, tab, index) => (!tab.props.disabled ? enabledTabs.concat(index) : enabledTabs),
      []
    );
  ...
  handleOverflowNavClick = (_, { direction, multiplier = 10 }) => {
    ...
    const rightEdgeReached =
      direction === 1 && scrollLeft + clientWidth >= scrollWidth - this.OVERFLOW_BUTTON_OFFSET;
  ...
  render() {
    ...
    const tabContentWithProps = React.Children.map(tabsWithProps, (tab) => {
      const { id: tabId, children, selected, renderContent: Content = TabContent } = tab.props;
    ...
    const classes = {
      tabs: classNames(className, `${prefix}--tabs--scrollable`, {
        [`${prefix}--tabs--scrollable--container`]: type === 'container',
        [`${prefix}--tabs--scrollable--light`]: light,
      }),
      tablist: classNames(`${prefix}--tabs--scrollable__nav`),
      ...
      <>
        <div {...other} className={classes.tabs} onScroll={this.handleScroll}>
          <button
            ...
            onMouseDown={(event) => this.handleOverflowNavMouseDown(event, { direction: -1 })}
          ...
          <div role="tablist" tabIndex={-1} className={classes.tablist} ref={this.tablist}>
            {tabsWithProps}
          </div>
