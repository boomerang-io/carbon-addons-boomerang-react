...
test("Profile Settings success", async () => {
    ...
    const { findByText, getByLabelText, getByText, queryByText } = render(<QueryClientProvider client={queryClient}>
    ...
    fireEvent.click(btn);
    await waitFor(() => expect(queryByText(/For any questions or concerns about business personal information/i)).toBeNull());
});
...
test("Profile Settings error", async () => {
    ...
    const { findByText, findByLabelText, getByText } = render(<QueryClientProvider client={queryClient}>
      <ProfileSettings baseServiceUrl={baseServiceUrl} userName="Boomerang Joe" src="joe@ibm.com"/>
    </QueryClientProvider>);
    const userBtn = getByText("Boomerang Joe");
    fireEvent.click(userBtn);
