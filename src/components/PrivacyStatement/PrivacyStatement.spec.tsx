...
test("Privacy Statement error", async () => {
    ...
    const { getByText, getByRole, findByRole } = render(<QueryClientProvider client={queryClient}>
    ...
    await waitFor(() => (expect(getByText(/Failed to receive deletion request. Please try again./i)) as any).toBeInTheDocument());
});
