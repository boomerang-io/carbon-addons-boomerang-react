    ...
    >
      {(formikProps) => {
        ...
        const dataDrivenInputs = finalInputs.map((input) => {
          ...
          const inputValue = values[key] !== undefined && (Object.values(TEXT_INPUT_TYPES).includes(type) || type === TEXT_AREA_TYPES.TEXT_AREA || type === TEXT_EDITOR_TYPES.TEXT_EDITOR) ? values[key].toString() : values[key];
