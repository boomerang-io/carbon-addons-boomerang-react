...
export const findMenuItemNode = (index: any) => {
...
export const assertMenuOpen = (mockProps: any) => {
  // @ts-expect-error TS(2304): Cannot find name 'expect'.
  expect(findMenuNode().childNodes.length).toBe(mockProps.items.length);

  // @ts-expect-error TS(2304): Cannot find name 'expect'.
  expect(findMenuIconNode()).toHaveClass(`${prefix}--list-box__menu-icon--open`);
...
export const assertMenuClosed = () => {
  ...
  // @ts-expect-error TS(2304): Cannot find name 'expect'.
  expect(findMenuIconNode()).not.toHaveClass(`${prefix}--list-box__menu-icon--open`);
  // @ts-expect-error TS(2304): Cannot find name 'expect'.
 ...
 */
export const generateItems = (amount: any, generator: any) =>
  Array(amount)
    .fill(null)
    .map((_, i) => generator(i));
